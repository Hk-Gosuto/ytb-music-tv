...
- UDP 局域网发现和持久化设备身份
- 匿名访问公共目录；配置浏览器 Cookie 后可访问个人媒体库
...
docker compose logs -f ytb-music-tv-server
```
...
搜索、公开首页和公开媒体播放不要求登录。个人媒体库需要浏览器 Cookie；当前版本没有登录 UI，需要在持久化目录中配置 `session.json` 后重启服务：

```json
{
  "cookie": "从已登录的 YouTube Music 浏览器请求中复制的 Cookie 请求头",
  "poToken": "",
  "visitorData": "",
...
- Docker Compose 使用根目录的 `data/session.json`
- 直接运行服务端使用 `server/data/session.json`
- `poToken` 和 `visitorData` 为可选项

Cookie 等同于登录凭证，请勿提交到版本库、日志或公开分享。项目已在 `.gitignore` 中排除上述数据目录。
...
Docker Compose 将 TCP `4174` 和 UDP `4175` 暴露到宿主机，并把根目录的 `data/` 挂载到容器 `/data`。
